...
if ($transport->xpdo) {
    ...
    /** @var array $config */
    if (file_exists(MODX_BASE_PATH . 'Extras/SocialNetworks/_build/config.inc.php')) {
        $config = include(MODX_BASE_PATH . 'Extras/SocialNetworks/_build/config.inc.php');
    }
    
    $base = MODX_BASE_PATH . 'Extras/SocialNetworks/';
    $core = MODX_CORE_PATH;
    ...
    if (file_exists($base) && $cache) {
        ...
        $files = [
            'assets' => [
                'target' => $base . 'assets/components/socialnetworks',
                'production' => $assets . 'components/socialnetworks/'    
            ],
            'core' => [
                'target' => $base . 'core/components/socialnetworks',
                'production' => $core . 'components/socialnetworks/'  
            ]
        ];
